...
class BookType(db.Model):
    ...
    books = db.relationship('Book', backref='book_type_name', lazy='dynamic')

    def json_dump(self):
        return dict(id=self.id, type_name=self.type_name)

    def __repr__(self):
        return '<BookType %r>' % self.type_name


class BookGenre(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    genre = db.Column(db.String(100), index=True, nullable=False)
    type_name = db.Column(db.String(100), db.ForeignKey('book_type.type_name'), nullable=False)
    books = db.relationship('Book', backref='gender', lazy='dynamic')
...
class Book(db.Model):
    ...
    book_type = db.Column(db.String(100), db.ForeignKey('book_type.type_name'), nullable=False)
